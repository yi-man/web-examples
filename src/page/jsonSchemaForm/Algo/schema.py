from x_types import String, Bool, Integer, Float, Any
...
vertical_xgboost_label_trainer_rule = {
    ...
    "output": {
        ...
        "model": {
            "name": String("vertical_xgboost_[STAGE_ID].json")
        },
        "metric_train": {
    ...
    "train_info": {
        ...
        "train_params": {
            ...
            "early_stopping": {
                "key": OneOf("acc", "precision", "recall", "f1_score", "auc", "ks").set_default_index(-1).add_rule(lambda x, y: x in y["train_info"]["train_params"]["metric"].keys(), "should in metric"),  # 这里的key必须是在metric里配置过的key
